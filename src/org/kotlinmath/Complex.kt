...
interface Complex {
    ...
    operator fun times(z: Complex): Complex {
        return when {
            ...
            z.isInfinite() -> if (isZero()) NaN else INF
                ...
                val product1 = (re + im) * z.re
                val product2 = im * (z.re + z.im)
                val product3 = (re - im) * z.im
                complex(product1 - product2, product2 + product3)
            }
        }
    }
    ...
    fun asStringNoSmallPart(format: String = "", locale: Locale = Locale.getDefault()): String {
        return when (this) {
            NaN -> "NaN"
            INF -> "Infinity"
            else -> {
                val reTooSmall = abs(re) < 0.000001
                val reFormatted = when {
                    reTooSmall -> ""
                    format.isEmpty() -> re.toString()
                    else -> String.format(locale, format, re)
                }
                val imTooSmall = abs(im) < 0.000001
                val imFormatted = when {
                    im == 1.0 -> "i"
                    im == -1.0 -> "-i"
                    imTooSmall -> ""
                    format.isEmpty() -> "${im}i"
                    else -> "${String.format(locale, format, im)}i"
                }
                if (re == 0.0) {
                    if (im == 0.0) "0.0" else imFormatted
                } else {
                    when {
                        imTooSmall && reTooSmall -> "0"
                        im > 0.0 -> {
                            if (imTooSmall || reTooSmall) "$reFormatted$imFormatted"
                            else "$reFormatted+$imFormatted"
                        }
                        im < 0.0 -> "$reFormatted$imFormatted"
                        else -> reFormatted
                    }
 ...
 * @return the created complex number
 */
val complex: (re: Number, im: Number) -> Complex = { re, im -> DefaultComplex(re.toDouble(), im.toDouble()) }
